...
class GameStateServiceTest {
    ...
    void broadcastGameState_ShouldSendPrivateStateToEachPlayer() {
        ...
        // Should send private messages to each player using convertAndSendToUser
        for (Player player : testGame.getPlayers()) {
            String compositeName = player.getName() + ":" + GAME_ID;
    ...
    void broadcastGameState_WithThreePlayers_ShouldSendThreePrivateMessages() {
        ...
        // 1 public message + 3 name-private via convertAndSendToUser
    ...
    void multipleBroadcasts_ShouldAllSucceed() {
        ...
        // 3 public broadcasts via convertAndSend + 3*(2 name-private) via convertAndSendToUser
    ...
    // ---- WAL replay suppression tests ----

    @Test
    @DisplayName("broadcastGameState is suppressed during replay")
    void broadcastGameState_SuppressedDuringReplay() {
        com.pokergame.wal.WalContext.setGlobalReplayMode(true);
        try {
            gameStateService.broadcastGameState(GAME_ID, testGame);
            verify(messagingTemplate, never()).convertAndSend(anyString(), any(Object.class));
        } finally {
            com.pokergame.wal.WalContext.setGlobalReplayMode(false);
        }
    }

    @Test
    @DisplayName("broadcastShowdownResults is suppressed during replay")
    void broadcastShowdownResults_SuppressedDuringReplay() {
        com.pokergame.wal.WalContext.setGlobalReplayMode(true);
        try {
            gameStateService.broadcastShowdownResults(GAME_ID, testGame, testPlayers, 50);
            verify(messagingTemplate, never()).convertAndSend(anyString(), any(Object.class));
        } finally {
            com.pokergame.wal.WalContext.setGlobalReplayMode(false);
        }
    ...
    @Test
    @DisplayName("broadcastAutoAdvanceNotification is suppressed during replay")
    void broadcastAutoAdvanceNotification_SuppressedDuringReplay() {
        com.pokergame.wal.WalContext.setGlobalReplayMode(true);
        try {
            gameStateService.broadcastAutoAdvanceNotification(GAME_ID, testGame);
            verify(messagingTemplate, never()).convertAndSend(anyString(), any(Object.class));
    ...
    @Test
    @DisplayName("sendPlayerNotification is suppressed during replay")
    void sendPlayerNotification_SuppressedDuringReplay() {
        com.pokergame.wal.WalContext.setGlobalReplayMode(true);
        try {
            gameStateService.sendPlayerNotification(GAME_ID, "Player1", "test message");
            verify(messagingTemplate, never()).convertAndSend(anyString(), any(Object.class));
    ...
    @Test
    @DisplayName("broadcastGameEnd is suppressed during replay")
    void broadcastGameEnd_SuppressedDuringReplay() {
        com.pokergame.wal.WalContext.setGlobalReplayMode(true);
        try {
            gameStateService.broadcastGameEnd(GAME_ID, testPlayers.getFirst(), false);
            verify(messagingTemplate, never()).convertAndSend(anyString(), any(Object.class));
        } finally {
            com.pokergame.wal.WalContext.setGlobalReplayMode(false);
        }
